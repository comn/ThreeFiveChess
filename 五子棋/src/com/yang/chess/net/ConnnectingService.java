...
import static com.yang.chess.net.ConnectConstants.BROADCAST_EXIT;
import static com.yang.chess.net.ConnectConstants.BROADCAST_JOIN;
import static com.yang.chess.net.ConnectConstants.CHAT_ONE;
import static com.yang.chess.net.ConnectConstants.CONNECT_AGREE;
import static com.yang.chess.net.ConnectConstants.CONNECT_ASK;
import static com.yang.chess.net.ConnectConstants.CONNECT_REJECT;
import static com.yang.chess.net.ConnectConstants.MULTICAST_ERROR;
import static com.yang.chess.net.ConnectConstants.ON_EXIT;
import static com.yang.chess.net.ConnectConstants.ON_JOIN;
import static com.yang.chess.net.ConnectConstants.SOCKET_NULL;
import static com.yang.chess.net.ConnectConstants.UDP_DATA_ERROR;
import static com.yang.chess.net.ConnectConstants.UDP_IP_ERROR;
import static com.yang.chess.net.ConnectConstants.UDP_JOIN;
...
public class ConnnectingService {
    ...
    // 广播组地址
    private static final String MUL_IP = "192.168.47.255";
    ...
    /**
     * 启动连接程序，开启消息接收的两个线程和两个发送处理消息的Handler
     */
    public void start(){
        mUdpReceiver.start();
        ...
        public void run() {
            try {
                while (isInit){
                    ...
                    int type = buf[0]; //获取数据类型
                    // 从buffer中截取收到的数据
                    byte[] body = new byte[dataPacket.getLength() - 1];
                    //从数组buf中的第二个索引位拷贝元素到body数组中。
...
//                    其他玩家广播的ip,则该ip发送自己的数据消息（name+ip）,表示自己在线。而不是向整个网络广播消息。
