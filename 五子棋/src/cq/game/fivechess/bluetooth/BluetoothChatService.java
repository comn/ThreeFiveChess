...
package cq.game.fivechess.bluetooth;
...
public class BluetoothChatService {
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice
            device, final String socketType,Boolean isServer) {
...
//        后又同步执行mConnectThread
        ...
        Message msg = mHandler.obtainMessage(BluetoothConstants.MESSAGE_IMFOR);
                ...
                if (socket != null) {
                    synchronized (BluetoothChatService.this) {
                        ...
                        case STATE_CONNECTING:
                            // Situation normal. Start the connected thread.联机中
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    // Read from the InputStream  bytes为字节数，即buffer的长度
                    bytes = mmInStream.read(buffer);

                    // Send the obtained bytes to the UI Activity
                    mHandler.obtainMessage(buffer[1], bytes, -1, buffer)
                            .sendToTarget();
                } catch (IOException e) {
